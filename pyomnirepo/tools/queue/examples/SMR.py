...
logging.basicConfig(
    ...
    level=logging.INFO,
    handlers=[logging.FileHandler(filename=file_path), logging.StreamHandler()],
)


def log(
    database: str, start_time, exception: Exception = None, offset: int = None
) -> None:
    """
    Logs exceptions
    """
...
async def read_postgres(conn: any, redis: any, offset: int) -> None:
    ...
    try:
        ...
        try:
            ...
            log(database="PostgreSQL+Redis", offset=offset, start_time=start_time)
...
async def write_mongo(collection: any, redis: any) -> None:
    ...
    try:
        ...
        log(database="Redis", start_time=start_time, exception=exception)
        ...
        try:
            ...
            try:
                ...
                log(database="MongoDB", offset=offset, start_time=start_time)
...
def run_read_postgres_courotines():
    ...
    async def read_coroutines():
        ...
        conn = await asyncpg.connect(
            DATABASE_82.replace("postgresql+asyncpg", "postgresql")
        )

        coros = [read_postgres(conn, redis, offset) for offset in offsets()]
    ...
    try:
        ...
        log(database="PostgreSQL", start_time=start_time, exception=exception)
        ...
        if loop.is_closed():
            try:
                ...
                log(database="Redis", start_time=start_time, exception=exc)
...
def run_write_mongo_loop():
    ...
    loop.run_until_complete(write_mongo_courotine(collection, loop))


if __name__ == "__main__":
    p1 = mp.Process(target=run_read_postgres_courotines, name="read_from_postgres")
    p2 = mp.Process(target=run_write_mongo_loop, name="write_to_mongo")

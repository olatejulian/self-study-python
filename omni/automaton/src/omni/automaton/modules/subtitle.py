...
class SubRipFile:
    ...
    def __repl(
        ...
        def delay_repl(match: re.Match[str], delay: FloatSecond) -> str:
            start_time, end_time = cls.__get_time_in_seconds_from_sub_rip_time_format_str(match)

            start_time = cls.__delay_time(start_time, delay)

            end_time = cls.__delay_time(end_time, delay)

            return cls.__convert_seconds_to_sub_rip_time_format_str(start_time, end_time)
    ...
    @staticmethod
    def series_subtitle_name_format(name: str, episode: str, locale: str, extension: str) -> str:
